...
import { HungerLevel, AIReview } from "../types";
...
export const analyzePreachingNotes = async (notes: string): Promise<AIReview> => {
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          hungerLevel: {
            type: Type.STRING,
            description: "Spiritual hunger level: Low, Medium, or High",
          },
          suggestedVerse: {
            type: Type.STRING,
            description: "A relevant Bible verse for the next visit.",
          },
          suggestedNextAction: {
            type: Type.STRING,
            description: "Practical next step for follow-up.",
          },
          summary: {
            type: Type.STRING,
            description: "A 2-sentence executive summary of the encounter.",
          }
        },
  ...
  try {
    // Directly access the .text property of GenerateContentResponse to get the string output.
